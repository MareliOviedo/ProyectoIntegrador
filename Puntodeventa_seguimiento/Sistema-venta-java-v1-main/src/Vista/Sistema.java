...
public final class Sistema extends javax.swing.JFrame {
    ...
    private javax.swing.JTextField txtPagaConVenta;
    ...
    private void btnEditarproActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEditarproActionPerformed
        ...
        if ("".equals(txtCodigoPro.getText()) || "".equals(txtDesPro.getText())
            ...
            JOptionPane.showMessageDialog(null, "Completa código, descripción, proveedor, stock y precio.");
            return;
        }

        try {
            Combo itemP = (Combo) cbxProveedorPro.getSelectedItem();
            Combo itemC = obtenerCategoriaProducto();
            if (itemC == null || itemC.getId() <= 0) {
                JOptionPane.showMessageDialog(this, "No se pudo asignar una categoría. Revisa la tabla categorias.");
                return;
            }
            ...
            if (proDao.ModificarProductos(pro)) {
                ...
                JOptionPane.showMessageDialog(null, "No se pudo modificar el producto. Revisa la base de datos.");
            }
    ...
    private void btnGuardarproActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnGuardarproActionPerformed
        ...
        try {
            ...
            if (proDao.RegistrarProductos(pro)) {
                ...
                JOptionPane.showMessageDialog(null, "No se pudo registrar el producto. Revisa la base de datos.");
            }
        } catch (NumberFormatException e) {
            JOptionPane.showMessageDialog(this, "Stock y precio deben ser números válidos.");
        }
    ...
    private void btnGenerarVentaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnGenerarVentaActionPerformed
        ...
        double porcentajeDescuento = obtenerPorcentajeDescuentoVenta();
        double montoDescuento = obtenerMontoDescuentoVenta();
        double totalFinal = obtenerTotalConDescuento();
        double pagoCon = obtenerPagoVenta();
        double cambio = pagoCon - totalFinal;

        if (txtPagaConVenta == null || txtPagaConVenta.getText().trim().equals("")) {
            JOptionPane.showMessageDialog(this, "Escribe con cuánto va a pagar el cliente.");
            txtPagaConVenta.requestFocus();
            return;
        }

        if (pagoCon < totalFinal) {
            JOptionPane.showMessageDialog(this, "El pago no alcanza para cubrir el total. Faltan " + dinero(totalFinal - pagoCon));
            txtPagaConVenta.requestFocus();
            return;
        }

        if (!RegistrarVenta()) {
            JOptionPane.showMessageDialog(this, "No se pudo guardar la venta. Revisa el mensaje de error de la base de datos.");
            return;
        }
        int idVentaGenerada = RegistrarDetalle();
        if (idVentaGenerada <= 0) {
            return;
        }
        ActualizarStock();

        String ticket = generarTicketPantalla(idVentaGenerada, subtotal, porcentajeDescuento,
                montoDescuento, totalFinal, pagoCon, cambio);
        mostrarTicketPantalla(ticket);

        LimpiarTableVenta();
        LimpiarClienteventa();
        LimparVenta();
        LimpiarPagoDescuentoVenta();
    ...
    private void txtCantidadVentaKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtCantidadVentaKeyPressed
        if (evt.getKeyCode() == KeyEvent.VK_ENTER) {
            if (txtIdPro.getText().trim().equals("")) {
                JOptionPane.showMessageDialog(null, "Primero busca o selecciona un producto.");
                txtCodigoVenta.requestFocus();
                return;
            }
            if (txtCantidadVenta.getText().trim().equals("")) {
            ...
            try {
                ...
                if (stock < cant) {
                    JOptionPane.showMessageDialog(null, "Stock no disponible. Disponible: " + stock);
                    txtCantidadVenta.requestFocus();
                    return;
                }
                ...
                for (int i = 0; i < TableVenta.getRowCount(); i++) {
                    if (Integer.parseInt(TableVenta.getValueAt(i, 0).toString()) == id) {
                        JOptionPane.showMessageDialog(null, "El producto ya está registrado en la venta.");
                        return;
                    }
                }
                ...
                JOptionPane.showMessageDialog(this, "Cantidad, precio y stock deben ser números válidos.");
            }
        }
    ...
    private void TotalPagar() {
        ...
        double montoPagar = obtenerTotalConDescuento();
        LabelTotal.setText(String.format("%.2f", montoPagar));
        calcularCambioVenta();
    }

    private double obtenerPorcentajeDescuentoVenta() {
        try {
            if (txtDescuentoVenta == null || txtDescuentoVenta.getText().trim().equals("")) {
                return 0.00;
            }

            double porcentaje = Double.parseDouble(txtDescuentoVenta.getText().trim());

            if (porcentaje < 0) {
                JOptionPane.showMessageDialog(this, "El porcentaje de descuento no puede ser negativo.");
                return 0.00;
            }

            if (porcentaje > 100) {
                JOptionPane.showMessageDialog(this, "El porcentaje de descuento no puede ser mayor a 100%.");
                return 0.00;
            }

            return porcentaje;
        } catch (NumberFormatException e) {
            JOptionPane.showMessageDialog(this, "El descuento debe ser un número válido. Ejemplo: 20 para aplicar 20%.");
            return 0.00;
        }
    }

    private double obtenerMontoDescuentoVenta() {
        double porcentaje = obtenerPorcentajeDescuentoVenta();
        return Totalpagar * (porcentaje / 100.00);
    }

    private double obtenerTotalConDescuento() {
        double monto = Totalpagar - obtenerMontoDescuentoVenta();
        if (monto < 0) {
            monto = 0.00;
        }
        return monto;
    ...
    private double obtenerPagoVenta() {
        try {
            if (txtPagaConVenta == null || txtPagaConVenta.getText().trim().equals("")) {
                return 0.00;
            }
            return Double.parseDouble(txtPagaConVenta.getText().trim());
        } catch (NumberFormatException e) {
            JOptionPane.showMessageDialog(this, "El pago debe ser un número válido.");
            return 0.00;
        }
    }

    private double obtenerCambioVenta() {
        double pago = obtenerPagoVenta();
        double total = obtenerTotalConDescuento();
        return pago - total;
    }

    private void calcularCambioVenta() {
        ...
        try {
            if (txtPagaConVenta != null && !txtPagaConVenta.getText().trim().equals("")) {
                pago = Double.parseDouble(txtPagaConVenta.getText().trim());
            }
        } catch (NumberFormatException e) {
        ...
        double cambio = pago - obtenerTotalConDescuento();
        if (cambio < 0) {
            txtCambioVenta.setText("0.00");
        } else {
            txtCambioVenta.setText(String.format("%.2f", cambio));
        }
    }
    ...
    private void LimpiarPagoDescuentoVenta() {
        if (txtDescuentoVenta != null) {
            txtDescuentoVenta.setText("0");
        }
        if (txtPagaConVenta != null) {
            txtPagaConVenta.setText("");
        }
        if (txtCambioVenta != null) {
            txtCambioVenta.setText("0.00");
        }
    ...
    private String generarTicketPantalla(int idVenta, double subtotal, double porcentajeDescuento,
            double montoDescuento, double totalFinal, double pagoCon, double cambio) {

        StringBuilder ticket = new StringBuilder();
        ticket.append("                 TICKET DE VENTA\n");
        ticket.append("=============================================\n");
        ticket.append("Folio: ").append(idVenta).append("\n");
        ticket.append("Fecha: ").append(new java.text.SimpleDateFormat("dd/MM/yyyy HH:mm:ss").format(new Date())).append("\n");
        ticket.append("Vendedor: ").append(LabelVendedor.getText()).append("\n");
        ticket.append("Cliente: ").append(txtNombreClienteventa.getText()).append("\n");
        ticket.append("=============================================\n");
        ticket.append(String.format("%-18s %5s %9s %9s\n", "Producto", "Cant", "P.Unit", "Importe"));
        ticket.append("---------------------------------------------\n");

        for (int i = 0; i < TableVenta.getRowCount(); i++) {
            String producto = TableVenta.getValueAt(i, 1).toString();
            if (producto.length() > 18) {
                producto = producto.substring(0, 18);
            }
            String cantidad = TableVenta.getValueAt(i, 3).toString();
            double precio = Double.parseDouble(TableVenta.getValueAt(i, 4).toString());
            double importe = Double.parseDouble(TableVenta.getValueAt(i, 5).toString());

            ticket.append(String.format("%-18s %5s %9.2f %9.2f\n", producto, cantidad, precio, importe));
        }

        ticket.append("---------------------------------------------\n");
        ticket.append(String.format("%31s %11s\n", "Subtotal:", dinero(subtotal)));
        ticket.append(String.format("%31s %10.2f%%\n", "Descuento:", porcentajeDescuento));
        ticket.append(String.format("%31s %11s\n", "Monto descuento:", dinero(montoDescuento)));
        ticket.append(String.format("%31s %11s\n", "TOTAL:", dinero(totalFinal)));
        ticket.append(String.format("%31s %11s\n", "Paga con:", dinero(pagoCon)));
    ...
    private void mostrarTicketPantalla(String ticket) {
        javax.swing.JTextArea areaTicket = new javax.swing.JTextArea(ticket);
        areaTicket.setEditable(false);
        areaTicket.setFont(new java.awt.Font("Monospaced", java.awt.Font.PLAIN, 12));
        javax.swing.JScrollPane scrollTicket = new javax.swing.JScrollPane(areaTicket);
        scrollTicket.setPreferredSize(new java.awt.Dimension(470, 430));

        JOptionPane.showMessageDialog(this, scrollTicket, "Ticket de venta", JOptionPane.INFORMATION_MESSAGE);
    }

    private boolean RegistrarVenta() {
        int cliente = Integer.parseInt(txtIdCV.getText());
        String vendedor = LabelVendedor.getText();
        double descuento = obtenerMontoDescuentoVenta();
        double monto = obtenerTotalConDescuento();

        v.setCliente(cliente);
        v.setVendedor(vendedor);
        v.setTotal(monto);
        v.setFecha(fechaActual);
        return Vdao.RegistrarVenta(v, descuento);
    }

    private int RegistrarDetalle() {
        int id = Vdao.IdVenta();
        if (id <= 0) {
            JOptionPane.showMessageDialog(this, "No se encontró el folio de la venta. No se guardará el detalle.");
            return 0;
        }
        ...
        int cliente = Integer.parseInt(txtIdCV.getText());
        Vdao.pdfV(id, cliente, obtenerTotalConDescuento(), LabelVendedor.getText());
        return id;
    }
    ...
    private Combo obtenerCategoriaProducto() {
        if (cbxCategoriaPro != null && cbxCategoriaPro.getSelectedItem() instanceof Combo) {
            return (Combo) cbxCategoriaPro.getSelectedItem();
        }

        int idCategoria = proDao.RegistrarCategoria("General");
        if (idCategoria <= 0) {
            return null;
        }
        ...
        return new Combo(idCategoria, "General");
    }
    ...
    private void configurarDescuentoVentaUI() {
        javax.swing.JLabel lblDesc = new javax.swing.JLabel("Desc. %:");
        lblDesc.setFont(new java.awt.Font("Tahoma", 1, 12));
        jPanel2.add(lblDesc, new org.netbeans.lib.awtextra.AbsoluteConstraints(520, 345, 65, 20));
        ...
        txtDescuentoVenta.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyTyped(java.awt.event.KeyEvent evt) {
                event.numberDecimalKeyPress(evt, txtDescuentoVenta);
            }
        ...
        jPanel2.add(txtDescuentoVenta, new org.netbeans.lib.awtextra.AbsoluteConstraints(585, 342, 60, 28));

        javax.swing.JLabel lblPaga = new javax.swing.JLabel("Paga con:");
        lblPaga.setFont(new java.awt.Font("Tahoma", 1, 12));
        jPanel2.add(lblPaga, new org.netbeans.lib.awtextra.AbsoluteConstraints(650, 345, 70, 20));

        txtPagaConVenta = new javax.swing.JTextField();
        txtPagaConVenta.setHorizontalAlignment(javax.swing.JTextField.CENTER);
        txtPagaConVenta.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyTyped(java.awt.event.KeyEvent evt) {
                event.numberDecimalKeyPress(evt, txtPagaConVenta);
            }
        ...
        jPanel2.add(txtPagaConVenta, new org.netbeans.lib.awtextra.AbsoluteConstraints(720, 342, 75, 28));

        javax.swing.JLabel lblCambio = new javax.swing.JLabel("Cambio:");
        lblCambio.setFont(new java.awt.Font("Tahoma", 1, 12));
        jPanel2.add(lblCambio, new org.netbeans.lib.awtextra.AbsoluteConstraints(650, 415, 70, 20));

        txtCambioVenta = new javax.swing.JTextField("0.00");
        txtCambioVenta.setEditable(false);
        txtCambioVenta.setHorizontalAlignment(javax.swing.JTextField.CENTER);
        txtCambioVenta.setBackground(new java.awt.Color(184, 202, 213));
        jPanel2.add(txtCambioVenta, new org.netbeans.lib.awtextra.AbsoluteConstraints(720, 412, 75, 28));

        // Botón Calcular removido: el total y el cambio se calculan automáticamente.
